...
export async function initCommand(options: {
  ...
  if (fs.existsSync(targetDir)) {
    console.log(
      ...
      `.layout/ already exists in this directory.`
    );
    console.log(`Use ${chalk.cyan("layout-context use <kit>")} to switch kits.`);
    return;
  }
  ...
  console.log();
  console.log(
    `  ${chalk.dim("1.")} Edit ${chalk.cyan(`.layout/${DESIGN_MD_FILE}`)} with your design system`
  );
  console.log(
...
async function initWithKit(kitName: string, targetDir: string): Promise<void> {
  ...
  if (bundledPath) {
    ...
    console.log(
      chalk.green("✓"),
      `Initialised .layout/ with the ${chalk.bold(kitName)} kit.`
    );
    return;
  }
  ...
  console.log(
    `Run ${chalk.cyan("layout-context list")} to see available kits.`
  );
}
...
async function initBlank(targetDir: string): Promise<void> {
  ...
  console.log(
    chalk.green("✓"),
    "Initialised .layout/ with a blank design system template."
  );
}
