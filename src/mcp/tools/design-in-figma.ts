    ...
    if (!kit) {
      return {
        content: [
          {
            type: "text" as const,
            text: "No design system kit loaded. Run `npx @superduperui/context init` to set up a kit first, or import one with `npx @superduperui/context import <path-to-zip>`.",
          },
        ],
      };
    }
