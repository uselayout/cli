...
This guide walks through the complete SuperDuper design system loop:

1. **Extract** Linear's design system via AI Studio
...
- **AI Studio** running locally (`npm run dev` → `localhost:3000`)
- **@superduperui/context** built (`npm run build` in this repo)
...
1. Open AI Studio at `http://localhost:3000`
...
### Option A: Import AI Studio Export (Full Extraction)
...
# Import the AI Studio export
npx @superduperui/context import ~/Downloads/linear-export.zip
```
...
```json
{
  "mcpServers": {
    "design-context": {
      "command": "node",
      "args": ["/path/to/superduperui-context/dist/bin/cli.js", "serve"],
...
**The loop:**
```
AI Studio (extract) → DESIGN.md → Agent (generate) → Preview (:4321)
...
*Generated by SuperDuper — superduperui.com*
